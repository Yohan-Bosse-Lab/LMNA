...
output:
  html_document:
    number_sections: F
params:
  datapath: 'C:/Users/renseb01/Documents/Steinberg_Christian/LMNA'
---
...
annotation = data.frame(x=c(156107442/1000000,156083176/1000000),y=c(0.4,-0.05),label = c('rs267607581','LMNA'))
...
pdf(file.path(params$datapath,'results/Figure5_nucleotide_diversity.pdf'),width = 14,height = 8)
...
clinical_family = merge(clinical,family, by.x = 'dossier', by.y = 'id')
clinical_family$dossier = as.character(clinical_family$dossier)

#remove the ones I don't have genotypes for
clinical_family = clinical_family[clinical_family$dossier %in% colnames(chr1[[1]])[10:61],]

#create ad data.frame that will contain info on patients.
colnames_family = merge(data.frame(dossier = colnames(chr1[[1]])[10:61]),clinical_family[,colnames(clinical_family) %in% c('family_number','family','dossier')],by = 'dossier',all= T)
colnames_family$family[is.na(colnames_family$family)] = 'None'
...
for(n in 1:nrow(colnames_family)){
  ...
  if((length(temp) == 2) & (temp[1] != 'None')) colnames_family$family_anonyme[n] = paste0(substr(temp[1],1,1),' ',temp[[2]])
  if((length(temp) == 1) & (temp[1] != 'None')) colnames_family$family_anonyme[n] = substr(temp[1],1,1)
}


for(i in 1:3){
  chr1[[i]][,10:61] = chr1[[i]][,order(colnames_family$family_anonyme)+9]
#  colnames(chr1[[i]])[10:61] = paste0(colnames_family$family[order(colnames_family$family)],1:52)
}
...
legend_colors = list(c('orange','gray85'),c('orange','gray85'),c('blue','darkred','gray85'))
...
for(i in 1:3){
  ...
  #chr_medium$sampleID = sapply(strsplit(x = chr_medium$sample, split = '_'),'[',1)
 #chr_medium$sampleID = factor(chr_medium$sampleID, levels = chr_medium$sampleID[1:52])
...
galign[[i]] = 
  ggplot(chr_medium,aes(x=POS,y=1,col = value,fill = value)) +
  geom_tile(size = 0.4) + 
  geom_vline(xintercept = 156.107442,color = 'black',linewidth = 1) +
 # facet_grid(rows = vars(sampleID)) +
  facet_grid(sampleID ~ .,switch = 'y') +
  labs(y = NULL) + 
  guides(y = "none",col = 'none',size = 'none') +
  ...
  theme_classic() +
  xlab('Chromosome 1 position (Mb)') +
  theme(strip.text.y.left = element_text(angle = 0,size = 5),panel.spacing.y = unit(0.1, "lines")) +
  ggtitle(c('phased chromosome 1 (haplotype 1, reference rs267607581 allele) ~ 25MB window','phased chromosome 1 (haplotype 2, mutated rs267607581 allele) ~ 25MB window','Diploid chromosome 1 ~ 25MB window')[i])
}



pdf(file.path(params$datapath,'results/Figure2_diploid_alignment.pdf'),width = 14,height = 8)
galign[[3]]
dev.off()


pdf(file.path(params$datapath,'results/Figure3_reference_alignment.pdf'),width = 14,height = 8)
galign[[1]]
dev.off()

pdf(file.path(params$datapath,'results/Figure4_mutated_alignment.pdf'),width = 14,height = 8)
galign[[2]]
dev.off()
```
...
vcf <- read.vcfR("LMNA_52_phased/chr1_h12.vcf")
...
plot_heatmap = Heatmap(d_mat,
        ...
        column_names_rot = 45,
        column_title = "Genetic distance (Nei's) heatmap, Chromosome 1",
        heatmap_legend_param = list(title = "Genetic Distance"))



pdf(file.path(params$datapath,'results/Figure1_heatmap.pdf'),width = 14,height = 8)
plot_heatmap
dev.off()
...
#read LD data
ld = read.table('LMNA_52_phased/LD_rs267607581.list.hap.ld',header = T)
...
pdf(file.path(params$datapath,'results/Figure6_ld_plot.pdf'),width = 6,height = 4)
ld_plot
dev.off()
```
...
region =  read.table('LMNA_52_phased/region_rs267607581.hap.ld', header= T)
...
rownames(matrix)[337] = 'LMNA\n(156 MB)'


pdf(file.path(params$datapath,'results/Figure7_ld_heatmap.pdf'),width = 6,height = 4)

Heatmap(matrix,cluster_rows = F,cluster_columns = F,row_names_gp = gpar(fontsize = 5),column_names_gp = gpar(fontsize = 5),name = "abs (D')", col = colorRamp2(c(0, 0.5, 1), c("darkblue", "white", "darkorange")),row_names_side = "left",column_title = "LD map (chromosome 1: 118-147MB)")
