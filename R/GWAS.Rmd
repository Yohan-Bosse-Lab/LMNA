...
manhattan_plot = ggplot(results_subset_forggplot, aes(x = BPcum, y = -log10(pvalue))) +
    ...
    ggtitle('Manhattan plot: SNP genotypes ~ severity')
...
clinical$BAG3[clinical$BAG3 =='BB (homozygous)'] =  'AG (heteroz.) &\nGG (homozygous)'
...
bag3_ggplot =
  ggplot(bag3) + 
    ...
    ggtitle('BAG3 genotypes ~ severity\n(Odds-ratio of G allele = 3.6, p = 0.07)') +
    ylab('Frequency') +
    xlab('') +
    theme(axis.text.x = element_text(size=8),plot.title = element_text(size = 12)) + 
    theme_bw()

#save plot
pdf(file.path(params$resultspath,paste0('July2025/Figure5_disease_severity.pdf')),width = 6,height = 8)
(manhattan_plot / (bag3_ggplot | plot_spacer()) ) + plot_annotation()
dev.off()
```
