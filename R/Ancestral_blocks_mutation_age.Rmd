...
colnames_family = merge(data.frame(id = colnames(chr1[[1]])[10:61]),clinical_family[,colnames(clinical_family) %in% c('family_number','family','id','severity','Record.ID')],by = 'id',all= T)
...
for(m in 1:2){ #m is to choose the reference vs mutated haplotype.
    ...
    #Summarise the ancestral segment lengths in a data frame
    ...
    segments = merge(segments,colnames_family[,c(1,2)],by.x = 0,by.y= 'id')
    #segments = segments[-s,]
    ...
    #Plot the results in the same order as the families
    segments = segments[as.numeric(rownames(colnames_family)),]
    segments_to_plot = segments[52:1,]
    segments_to_plot$type = 'Region of continuous\nhaplotype sharing' 
    segments_to_plot$ymin = seq(0.1,51.1,by =1)
    segments_to_plot$ymax = seq(0.85,52, by =1)

    #Background chromosomal segments
    segments_bg = segments[52:1,]
...
#    if(m == 2) {segments_bg$V1 = 50; segments_bg$V2 = 200}
    ...
    segments_bg$ymin = seq(0.75,52,by =0.5)[c(TRUE,FALSE)]
    segments_bg$ymax = seq(0.25,52,by =0.5)[c(TRUE,FALSE)]

    #
    segments_to_plot = rbind(segments_bg,segments_to_plot)
   # if(m == 2) 
    segments_to_plot = rbind(segments_to_plot,list('RN',156.0,156.190,'LMNA','LMNA',0.15,50.85))
    ...
    xbreaks=c(50,75,100,125,150,175,200)
      ...
      gcolor = brewer.pal(10,'Paired')[c(2,8,10)]
      segments_color = c(rep("black",104),gcolor[3])
   #   if(m==1) segments_color = segments_color[1:104]

    ancestral_plots[[m]] = ggplot(segments_to_plot_all[[m]]) + 
      geom_rect(aes(xmin = V1, xmax = V2, ymin = ymin, ymax = ymax,fill = type), alpha = c(rep(0.5,52),rep(1,c(53,53)[m])), color = segments_color) +
      xlab('Chromosome 1 (Mb)') +
      ylab('Patients') +
      scale_fill_discrete('Chromosomal segments',type = gcolor) +
      ...
      scale_y_continuous(breaks=c(0:51)+0.475,labels = segments_to_plot_all[[m]]$Record.ID[1:52], limits = c(0,52),expand = c(0.01,0.01)) + 
      theme_classic() +
      theme(legend.position = c('inside','none')[m],legend.position.inside = c(0.3, 0.8),axis.text.y = element_text(size=8),plot.title = element_text(size = 12)) +
...
#pdf(file.path(params$resultspath,paste0('July2025/Figure6_age.pdf')),width = 12,height = 16)
